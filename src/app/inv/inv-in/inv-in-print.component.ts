  ...
  template: `
    ...
    <div class="header">
      <div class="p-grid" style="margin-top:0">
        <div class="p-col-12">
          <div style="font-size: 1.5em;font-weight: bold">ใบรับวัสดุ</div>
        </div>
        <div class="p-col-4">เลขที่รับ: {{inv.StInvID}}<br>ผู้จำหน่าย: {{inv.SupplierName}}</div>
        ...
        <div class="p-col-4 rtx" style="padding-right: 1.5em">
          <div *ngIf="!inv.StInvUpdate">รอรับเข้าคลัง</div>
          <div *ngIf="inv.StInvUpdate">รับเข้าคลังแล้ว<br>{{inv.StInvDateUpdate | longDate:'th'}}</div>
        </div>
        ...
        <div class="p-col-12" style="padding-right: 1.5em;font-size: 0.8em;">
          <table class="collapse ctx">
            <tr>
              ...
              <td style="width:10%">Lot No.</td><td style="width:10%">วันหมดอายุ</td><td style="width:12%">จำนวนรับ</td>
...
export class ProductInvInPrintComponent implements OnInit {
  ...
  ngOnInit() {
    this.inv.SupplierName = this.globalService.supplierTable.find(x => x.value === this.inv.StInvCust).label;
