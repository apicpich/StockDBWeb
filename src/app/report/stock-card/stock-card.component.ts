...
export class StockCardComponent implements OnInit, OnDestroy {
  ...
  selectFromLeftPanel() {
    ...
    this.reportService.stockCardReport({ query: this.query }).subscribe(([drugItem, st, ot]: any) => {
      ...
      if (drugItem.length) {
        ...
        for (let i = data.length - 1; i >= 0; i--) { // loop from bottom
          ...
          if (rec.mode) {
            rec.invName = this.globalService.customerTable.find(x => x.value === rec.StInvCust).label;
            rec.out = rec.DtAmount * rec.DtPack
            this.sumOut += rec.out
            curAmount += rec.out
            curValue += rec.DtAmount * rec.DtPrice
          } else {
            rec.invName = this.globalService.supplierTable.find(x => x.value === rec.StInvCust).label;
