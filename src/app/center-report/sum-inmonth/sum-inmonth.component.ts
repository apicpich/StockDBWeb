...
@Component({
  selector: 'app-sum-inmonth',
  templateUrl: './sum-inmonth.component.html',
  styleUrls: ['./sum-inmonth.component.scss']
})
export class SumINMonthComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.menuService.setTitle('4-10')
  ...
  DrugType: any; StInvBudget: any; StInvMethod: any;
  DtCoPO: any; DtInnovate = false;
  ...
  sumAllTotal() {
    return new Promise((resolve, reject) => {
      ...
      criteria += "[StInvUpdate] = True"
      const query = {
        field: 'Sum(CDbl([DtAmount])*IIf([StInvTaxStatus]=2,([DtPrice]-[DtDis])*(1+[StInvTaxValue]/100),[DtPrice]-[DtDis])) AS GndTotal',
  ...
  sumINMonthDataSearch() {
    ...
    this.centerReportService.getSumINMonth(payload).subscribe((data: any) => {
      this.GrandSTTotal = 0;
      this.paginatorSize = 0;
      if (data.length) {
        data.forEach(item => {
  ...
  sumINMonthCatDataSearch() {
    ...
    this.centerReportService.getSumINMonthCat(this.getCriteria()).subscribe((data: any) => {
