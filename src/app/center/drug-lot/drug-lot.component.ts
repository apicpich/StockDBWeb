...
@Component({
  selector: 'app-drug-lot',
  templateUrl: './drug-lot.component.html',
  styleUrls: ['./drug-lot.component.scss']
})
export class DrugLotComponent implements OnInit {
  ...
  ngOnInit() {
    this.dataService.getSupplier().then(() => {
  ...
  getLot(isAll) {
    return new Promise((resolve, reject) => {
      ...
      this.dataService.getProductLot(this.drugItem.DrugID, isAll).subscribe((result: any) => {
        this.myMsgService.clearLoading()
        if (result.length) {
          result.forEach(item => {
            const supp = this.dataService.SupplierTable.find(x => x.value === item.DrugDSupply)
